// Tento soubor se stará o připojení aplikace k databázi Supabase.

// Supabase je načteno z <script> tagu v index.html, což ho zpřístupní jako globální proměnnou `window.supabase`.
// Tento kód bezpečně zkontroluje, zda je knihovna dostupná, než se ji pokusí použít.
const SupabaseGlobal = (window as any).supabase;

// Kontrola, zda se knihovna Supabase vůbec načetla.
  ...
  if(rootDiv) {
    rootDiv.innerHTML = `
      <div style="padding: 40px; font-family: sans-serif; text-align: center; background-color: #fff; height: 100vh; display: flex; align-items: center; justify-content: center;">
        <div>
          <h1 style="color: #e53e3e; font-size: 24px;">Kritická chyba!</h1>
          <p style="font-size: 18px; margin-top: 10px;">Nepodařilo se načíst Supabase klienta.</p>
          <p style="font-size: 16px; margin-top: 20px; color: #718096;">Zkontrolujte, zda je zařízení připojeno k internetu a zda není blokován skript z cdn.jsdelivr.net.</p>
        </div>
      </div>
    `;
  }
  throw new Error("Supabase client not found. Check internet connection and script loading in index.html.");
}
